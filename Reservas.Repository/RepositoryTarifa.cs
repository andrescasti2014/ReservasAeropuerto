...
{
    public interface IRepositoryTarifa<T> : ICrud<T>
    {
    }
    public class RepositoryTarifa<T> : IRepositoryTarifa<T> where T : ITarifa
    {

        IDbContext<T> _ctx;

        public RepositoryTarifa(IDbContext<T> ctx)
        {
            _ctx = ctx;
        }
